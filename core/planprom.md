...
| **INTEL-EXCEL** | **Excel Market Research Upload — Admin-Driven Priority** | **Scope confirmed (Session 86 · 2026-05-17 · owner approved):** Admin อัพโหลด .xlsx 4 คอลัมน์ (ชื่อไทย, ชื่ออังกฤษ, ranking_need "10/10", ประเภท) → ระบบใช้ข้อมูลนี้เป็น Priority Layer สูงกว่า Google Suggest · **DB:** `intel_excel_ideas` (idea_text, title_en, ranking_need INT, engine_type, uploaded_at) · **API:** `POST /api/admin/intel/upload-excel` — parse xlsx → truncate+insert · **Component:** `ExcelUploader.tsx` ('use client') — file input + result badge · Upload button อยู่ใน header area ขวา (ข้างๆ cache badge) · **Priority Score:** ranking 10→weight 5 / 8-9→4 / 5-7→3 / 1-4→2 · สูงกว่า Google Suggest Level 1 (weight 3) · **Section S0:** แสดงเฉพาะเมื่อมีข้อมูล — group by ranking · match status ✅/🟠 · "+ สร้าง" pre-fill · **Files:** `migrations/20260517_intel_excel_ideas.sql` · `app/api/admin/intel/upload-excel/route.ts` · `app/admin/template-analytics/ExcelUploader.tsx` · `app/admin/template-analytics/page.tsx` | ✅ **Done · Live (Session 86 · 2026-05-17)** |
