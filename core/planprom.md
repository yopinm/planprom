...
| # | Test Case | Expected | Status |
|---|---|---|---|
| F1 | KPI cards | Total revenue · paid orders · unique buyers · downloads | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
| F2 | 14-day chart | daily sales bar chart แสดงถูกต้อง | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
| F3 | Per-template table | revenue/ชิ้น · sale count เรียง DESC | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
...
| J13 | ระบบ Request ฟอร์มด่วน — LINE-based + Unlock Code | **✅ UAT ผ่านครบ (Session 58 · 2026-05-13)** · **Concept:** ลูกค้าแอด LINE OA → แจ้งชื่อฟอร์ม → admin สร้างใน Form Builder → publish `is_request_only=true` → สร้าง Unlock Code 1-time → ส่ง code ทาง LINE → ลูกค้ากรอก code ที่ catalog → หยิบตะกร้า → จ่าย 50฿ → download · **Marketing:** floating bubble "หาฟอร์มไม่เจอ? 📌 Request ด่วน 50฿ ✅" (CSS keyframe 7s) · amber card หน้าโฮม · **Pricing:** `REQUEST_ONLY_PRICE = 50` constant ใน `lib/pricing.ts` (bypass volume tier) · **DB:** `templates.is_request_only BOOLEAN` + `promo_codes.template_id UUID` · **Unlock Code:** `REQ-XXXXXX` (randomBytes 3 hex) · max_uses=1 · expires 30 วัน · POST สร้างใหม่จะ deactivate code เดิมอัตโนมัติ · GET คืน active code ปัจจุบัน · **Cart UX:** item label "🔒 Request พิเศษ" · ราคา ฿50 ต่อชิ้น · ซ่อน upsell banner เมื่อมี request item · **Admin:** Request Only checkbox + price_baht input ในหน้า edit + "🔑 สร้าง Unlock Code" section แสดง code + คัดลอก + วันหมดอายุ · **Filter:** `fetchActivePromo` เพิ่ม `template_id IS NULL` ป้องกัน unlock code โผล่ใน promo banner หน้าโฮม | ✅ **Done · UAT ผ่าน (Session 58 · 2026-05-13)** |
| J14 | ระบบสมาชิก + Auto Push download link | ~~ยกเลิกแล้ว — owner decision 2026-05-13~~ | ❌ CANCELLED |
...
| J17 | ระบบสมาชิกรายเดือน (Subscription) | ~~ยกเลิกแล้ว — owner decision 2026-05-13~~ | ❌ CANCELLED |
...
| **RE-1** | **Report Engine** | **Scope confirmed (2026-05-13):** สร้าง engine ที่ 4 ให้ครบ loop — admin กรอก field → generate PDF → publish → cart → download ใช้ระบบเดิมทั้งหมด · **PDF Structure (8 sections):** S1 Cover Page (ชื่อ/subtitle/องค์กร/placeholder buyer/Report ID auto/วันที่/validity/confidential) · S2 TOC auto-gen + clickable · S3 Executive Summary (KPI 4 ช่อง + สรุป + ข้อค้นพบ + เสนอแนะเร่งด่วน) · S4 Introduction & Scope (วัตถุประสงค์/ขอบเขต/แหล่งข้อมูล/ช่วงเวลา/methodology/ข้อจำกัด) · S5 Core Content = Module C (Table) + Module F (Text) เท่านั้น MVP · S6 Conclusion (บทสรุป/ข้อค้นพบ/เสนอแนะ/ความเสี่ยง/คาดการณ์/score) · S7 Appendix (raw data/อ้างอิง/glossary/ผู้จัดทำ) · S8 Back Page (ลายเซ็น/QR Code/Disclaimer/copyright) · Header+Footer ทุกหน้า · Watermark + Report ID auto · Validity แสดงบน Cover เท่านั้น (ไม่ block download) · ชื่อผู้ซื้อ = placeholder ก่อน · **Files NEW:** `lib/engine-report-types.ts` · `lib/engine-report.ts` · `app/api/admin/templates/generate-report/route.ts` · `app/admin/templates/new/ReportEngineForm.tsx` · **Files MODIFY:** `lib/engine-types.ts` (+type) · `WizardClient.tsx` (+mode engine-report, document_type='report') · `app/templates/[slug]/page.tsx` (+preview card sky) · **FROZEN:** engine-checklist/planner/form · Cart/Checkout/Payment · proxy.ts · **ได้ฟรี:** `/templates?type=report` filter · homepage link · cart/payment/download flow · admin sales report TYPE_META | ✅ **Done · Live (2026-05-13)** |
| **ADM-FALLBACK-1** | **Emergency Admin Login (Supabase bypass)** | ✅ **CLOSED — superseded by ADM-RBAC-1 (2026-05-14)** · ฟีเจอร์นี้รวมอยู่ใน ADM-RBAC-1 Tier 2 แล้ว | ✅ Closed |
| J19 | **Template System Log Export** | **Spec (2026-05-08):** เพิ่มหน้า `/admin/templates/log-export` สำหรับ export ข้อมูล snapshot เพื่อส่งให้ Claude Code วิเคราะห์ปัญหาระบบ templates · **Export ประกอบด้วย:** (1) templates table snapshot (id, slug, title, tier, price_baht, status, document_type, pdf_path, sale_count, created_at) · (2) template_categories + template_category_links · (3) orders + order_items summary (last 30 วัน) · (4) recent errors จาก PM2 logs (ถ้าดึงได้) · (5) cart stats (active carts, cart_items count) · **Format**: JSON + copy-to-clipboard button · **Page**: server component query DB → client component copy/download · **Route**: `GET /admin/templates/log-export` | ✅ **Done · Live (Session 69 · 2026-05-14)** |
...
| **DAY1-UAT** | **Day One UAT after go-live — Reset & Verify Checklist** | **เป้าหมาย:** ตรวจสอบ + ล้าง test data ก่อนนับตัวเลขจริง หลัง go-live จริง · **A. ล้าง test data (SQL บน VPS):** · `orders` — DELETE rows ที่ owner ซื้อทดสอบเอง (ระบุ order_uid ก่อนลบ) → `DELETE FROM orders WHERE order_uid IN ('PP-...')` · `order_seq` — reset ลำดับเลขใหม่ถ้าต้องการเริ่ม 0001 → `ALTER SEQUENCE order_seq RESTART WITH 1` · `intel_snapshots` — TRUNCATE ถ้า snapshot dev testing ไม่ต้องการ → `TRUNCATE intel_snapshots` · `intel_fulfilled` — TRUNCATE test fulfilled clicks → `TRUNCATE intel_fulfilled` · `analytics_events` — DELETE pageviews จาก admin/dev IP → `DELETE FROM analytics_events WHERE properties->>'ip' = '...'` · `promo_codes.used_count` — reset โค้ดที่ใช้ทดสอบ → `UPDATE promo_codes SET used_count=0 WHERE code IN (...)` · `templates.sale_count` — recompute จาก order_items จริง → `UPDATE templates t SET sale_count = (SELECT COUNT(*) FROM order_items oi JOIN orders o ON o.id=oi.order_id WHERE oi.template_id=t.id AND o.status='paid')` · **B. UAT ตรวจสอบ หลัง reset:** · `/admin/report/sales` — Revenue ตรงกับ orders ที่เหลือ · ค่าธรรมเนียม ⓘ tooltip แสดงสูตร 1.7655% · `/admin/report/payments` — fee per order ถูกต้อง · `/admin/template-analytics` — intel_snapshots วันที่ 2+ โชว์ trend arrows 📈/📉 · Fulfilled tracking — กด "+ สร้าง" → chip ✅ ขึ้น · **C. บันทึก baseline:** · จด order_uid แรกที่เป็น real customer (ไม่ใช่ test) ไว้ใน docs · บันทึก go-live date ใน planprom.md · **FROZEN:** ห้ามลบ orders ที่มี status=paid และไม่ใช่ test โดยไม่ระบุ order_uid ชัดเจน | 🔲 **Pending (Session 63 · 2026-05-14)** |
...
| **PROMO-5** | **Promo UI Refresh — Pricing-Aware Display + Visual Upgrade** | **Scope (2026-05-14 · pending confirm):** · **Depends on:** UI-I (pricing ฿30/฿20/฿10 ต้อง live ก่อน) · **PromoCodeBanner (homepage):** แสดง savings ตาม tier ใหม่ชัดเจน — "ซื้อ 1 ชิ้น ฿30 → ฿XX หลังใช้โค้ด" / "ซื้อ 2–5 ชิ้น เริ่ม ฿20" + countdown progress bar (visual urgency แทน "หมดใน X วัน" แบบ text) + slots เหลือ (`max_uses - used_count` ถ้า max_uses ไม่ null) · **PromoCodeInput (checkout):** แสดง before/after breakdown per tier หลัง apply code (รายการ item + ราคาเดิม + ส่วนลด + ราคาสุดท้าย) · savings badge สีเขียวโดดเด่น + animation pulse เมื่อ code valid · **Admin promo form:** เพิ่ม "💡 Suggested discount" hint realtime — คำนวณจาก tier ปัจจุบัน (e.g., "฿5 off = -17% จากราคา ฿30") · **Files:** `components/promo/PromoCodeBanner.tsx` · `components/checkout/PromoCodeInput.tsx` · `app/admin/promo-codes/PromoCreateForm.tsx` | ✅ **Done · UAT ผ่าน (Session 65 · 2026-05-14)** |

**ลำดับ implement:** PROMO-3 ✅ → PROMO-1 ✅ → PROMO-2 ✅ → PROMO-4 ✅ → **(UI-I ก่อน) → PROMO-5**
...
| UI-H | **Analytics Page — Match Catalog Design** | ปรับ `/admin/template-analytics` ให้ design/layout match catalog style ปัจจุบัน · scope gate ก่อน: ดู component ปัจจุบัน + catalog reference → ระบุ diff → implement | ✅ **Done · Live (INTEL-E/SMART sessions · 2026-05-14)** |
| UI-I | **Pricing Restructure — Volume Tiers** | **✅ UAT ผ่าน (Session 62 · 2026-05-14)** · Scope confirmed (2026-05-14): เปลี่ยน pricing: ฿20/฿10/฿7 → ฿30/฿20/฿10 · **FROZEN:** Cart/Checkout/Payment/Omise flow core · engine forms · REQUEST_ONLY_PRICE (฿50 คงเดิม) · **A. Source of truth:** `lib/pricing.ts` — TIER_1:20→30 · TIER_2:10→20 · TIER_3:7→10 · **B. Tests rewrite:** `lib/pricing.test.ts` — ค่า expected ทุกตัว (1ชิ้น:20→30 · 2ชิ้น:30→50 · 5ชิ้น:60→110 · 6ชิ้น:67→120 · 10ชิ้น:95→160 · nextItemPrice · savedVsFullPrice · currentTierLabel) · **C. Bug fix — local duplicate functions (ยังมี ฿8 ผิด):** `app/checkout/page.tsx` ลบ local `tierPrice()` → import `PRICE_TIERS` จาก lib + fix request-only branch · `app/api/checkout/route.ts` ลบ local `tierPriceForItem()` → import จาก lib + fix `is_request_only` items ให้ใช้ `REQUEST_ONLY_PRICE` แทน tier position (bug: unit_price ใน order_items บันทึกผิด แม้ยอด Omise ถูก) · **D. UI text hardcode:** `app/page.tsx` — mock cards ฿20→฿30 · text "฿20 ชิ้นแรก · ฿10 · ฿7" · CTA "เริ่มต้นที่ ฿20"→฿30 · SEO metadata (pricing card ใช้ PRICE_TIERS แล้ว auto-update) · `app/templates/page.tsx` — pricing callout 3 จุด · filter value '20'→'30' · SEO metadata · `app/cart/page.tsx` — text "฿7/ชิ้น"→"฿10/ชิ้น" · `components/templates/TemplateListWithPreview.tsx` — TIER_LABEL standard/premium/ultra '฿20'→PRICE_TIERS.TIER_1 · `components/catalog/CatalogTemplateList.tsx` — TIER_LABEL เดียวกัน · **E. API guard:** `app/api/promo/validate/route.ts` — guard `< 7` → `< 10` (TIER_3 ใหม่) · **Files รวม 9 ไฟล์:** `lib/pricing.ts` · `lib/pricing.test.ts` · `app/checkout/page.tsx` · `app/api/checkout/route.ts` · `app/page.tsx` · `app/templates/page.tsx` · `app/cart/page.tsx` · `components/templates/TemplateListWithPreview.tsx` · `components/catalog/CatalogTemplateList.tsx` + `app/api/promo/validate/route.ts` | 🔄 **In Progress (Session 61)** |
...
| **INTEL-E** | **Catalog-Matched Analytics — 5 Smart Features** | **Scope confirmed (2026-05-14 · owner approved):** เพิ่ม intelligence 5 จุดใน `/admin/template-analytics` ให้ระบบรู้ว่า "สร้างอะไร → ใส่ catalog ไหน" · **FROZEN:** Google Suggest fetch logic · SEED_KEYWORDS · ALPHA_CHARS · AUDIENCE_RULES · NOISE · S1/S2 KPI+Revenue sections · DB schema · **Position: ระหว่าง S2 (Engine cards) กับ S3 (Priority list)** · **[NEW S2a] Catalog Performance Cards:** ยอดขายต่อ category จริง — ตาราง per-category: emoji+name · published template count · orders · revenue · status badge · **Query:** JOIN templates + template_category_links + template_categories + order_items + orders (status='paid') GROUP BY category · **[NEW S2b] Catalog Demand Heatmap:** Google demand hits vs template ที่มี per category — นับ ideas จาก `suggestCatalog()` ที่ match กับแต่ละ category → แสดง 🔥 hits count · template count · Gap status (📈 Gap สูง = hits≥5 แต่ template<2 / ✅ ดี / 🟠 ยังไม่มี) · ปุ่ม "+ เพิ่ม" per row → pre-fill · **[S3 Enhancement] Catalog Suggestion Badge ใน Priority Row:** แต่ละ row ใน "สร้างอะไรก่อน" → เพิ่ม "📂 → catalog: [ชื่อ]" badge ต่อท้าย engine badge · Logic: `suggestCatalog(idea, categories)` — scan คำใน idea string vs category.name+slug keywords (เช่น "ต่างประเทศ"→ท่องเที่ยว, "ร้าน"→ธุรกิจ, "ว่างงาน"→HR) · fetch `template_categories` ใน Phase 1 parallel · **[S3 Enhancement] Smart "+ สร้าง" Pre-fill URL:** ปุ่ม "+ สร้าง" ทุกจุด (S3 + S2b) → `href="/admin/templates/new?title=...&category=...&engine=..."` · `app/admin/templates/new/page.tsx` รับ searchParams → ส่งต่อ WizardClient · `WizardClient.tsx` รับ props → pre-fill title input + category select + engine mode tab · **[S4 Enhancement] Per-Catalog Coverage Badge:** เพิ่มแถว "แยกตาม Catalog" ใต้ coverage cards เดิมใน S4 · map AUDIENCE_RULES patterns → match กับ category.name → coverage % per catalog · badge: 📂 ชื่อ · X/Y covered · 🟢≥60% / 🟠≥30% / 🔴<30% · **Files: 3 ไฟล์** `app/admin/template-analytics/page.tsx` (S2a+S2b+S3+S4) · `app/admin/templates/new/page.tsx` (searchParams) · `app/admin/templates/new/WizardClient.tsx` (pre-fill) | ✅ **UAT ผ่าน (Session 62 · 2026-05-14)** |

| **INTEL-SMART** | **Analytics Smart Layer — 3 Features** | **Scope (Session 62 · 2026-05-14):** · **Feature 1 — Dedup+Cluster:** `normalizeIdea()` group ideas คล้ายกัน → badge `×N` · score = base×count → priority list แม่นขึ้น · **Feature 2 — Fulfilled Tracking:** `intel_fulfilled` table · `recordFulfilledAction` server action · "+ สร้าง" บันทึก idea → กรองออกจาก list ทันที · chip "✅ N fulfilled (30 วัน)" + collapsible history · **Feature 3 — Trend Score:** `intel_snapshots` table · snapshot daily (first page visit) · badge `📈+N` / `📉-N` / `→` ต่อ row · trend arrows โชว์วันที่ 2+ · **Files:** `app/admin/template-analytics/page.tsx` · `app/admin/template-analytics/actions.ts` · `migrations/20260514_intel_tables.sql` | ✅ **UAT ผ่าน (Session 86 · 2026-05-17)** |
| **INTEL-SCORE** | **Template Health Check — Quality Score** | **Scope confirmed (Session 72 · 2026-05-14):** Section ใหม่ท้าย `/admin/template-analytics` · Read-only จาก `templates` WHERE status='published' · ไม่มี DB ใหม่ · ไม่แตะ template list/edit · **คะแนน 5 มิติ รวม 100:** (1) ความครบถ้วน 30pt: thumbnail+description≥80+features≥3+category · (2) Engine data 30pt: engine_data ไม่ null + required fields ครบตาม engine_type · (3) ราคา/Payment 20pt: free→price=0/paid→price>0 · (4) Market fit 10pt: title match demand list · (5) SEO basics 10pt: slug≤60+description≥80 · **Grade:** 🟢≥80 / 🟡50-79 / 🔴<50 · **UI 2 ชั้น:** A=Leaderboard (summary chip + table sorted score ASC + issues + แก้ไข link) · C=คลิก row expand (per-dimension bar + suggestion "แก้ก่อน→score~X") · **Files:** `app/admin/template-analytics/page.tsx` · `app/admin/template-analytics/CopyButton.tsx` (new) | ✅ **Done · Live (Session 73 · 2026-05-15)** |
| **INTEL-SEO-PANEL** | **SEO Keyword Panel — Copy-ready Suggestions** | **Scope confirmed (Session 72 · 2026-05-14):** Section ใหม่ใน `/admin/template-analytics` ต่อท้าย Market Demand · Data source: Google Suggest ที่ดึงอยู่แล้ว — ไม่ fetch เพิ่ม · Output per keyword group (checklist/planner/form/report): target keyword (high-demand uncovered) + suggested blog title + meta description skeleton · Copy button: copy ทั้ง block → ไปกรอก `/admin/seo/new` ได้เลย · **Files:** `app/admin/template-analytics/page.tsx` + `CopyButton.tsx` | ✅ **Done · Live (Session 73 · 2026-05-15)** |
| **INTEL-BLOG-SCORE** | **Blog Post SEO Health Score** | **Scope (Session 76 · 2026-05-15 · รอ implement):** Card ใหม่ท้าย `/admin/template-analytics` · Read-only จาก `blog_posts` WHERE status='published' · ไม่มี DB ใหม่ · **คะแนน 5 มิติ รวม 100:** (1) Title /20: 30–60 ตัวอักษร (2) Description /20: ≥80 ตัวอักษร (≥160=เต็ม) (3) Content Length /25: ≥300 คำ (≥600=เต็ม) — นับจาก `content` field (4) Internal Links /20: มี href="/templates/" ≥1 จุดใน content (5) Tags /15: tags.length ≥2 · **Grade:** 🟢≥80 / 🟡50–79 / 🔴<50 · **UI:** leaderboard เรียง score ASC + expand row → per-dimension breakdown + hint "แก้ก่อน → +Xpt" + ลิ้งค์ "แก้ไข →" ไป `/admin/seo/[slug]/edit` · **Why:** blog post ที่คะแนนดี = Google crawl เจอเร็วขึ้นผ่าน sitemap → organic traffic · **Files:** `app/admin/template-analytics/page.tsx` | 🔲 **รอ implement** |

| **INTEL-EXCEL** | **Excel Market Research Upload — Admin-Driven Priority** | **Scope confirmed (Session 86 · 2026-05-17 · owner approved):** Admin อัพโหลด .xlsx 4 คอลัมน์ (ชื่อไทย, ชื่ออังกฤษ, ranking_need "10/10", ประเภท) → ระบบใช้ข้อมูลนี้เป็น Priority Layer สูงกว่า Google Suggest · **DB:** `intel_excel_ideas` (idea_text, title_en, ranking_need INT, engine_type, uploaded_at) · **API:** `POST /api/admin/intel/upload-excel` — parse xlsx → truncate+insert · **Component:** `ExcelUploader.tsx` ('use client') — file input + preview + confirm + ลบทั้งหมด · **Priority Score:** ranking 10→weight 5 / 8-9→4 / 5-7→3 / 1-4→2 · สูงกว่า Google Suggest Level 1 (weight 3) · **Section S0 ใหม่:** "📊 Research Database" แสดงบนสุดของ analytics — tier badge · ชื่ออังกฤษ · match status ✅/🟠 · "+ สร้าง" pre-fill · **Frozen:** fetchSuggestions · SEED_KEYWORDS · ALPHA_CHARS · NOISE constants · INTEL-SMART actions · **Files:** `migrations/20260517_intel_excel_ideas.sql` · `app/api/admin/intel/upload-excel/route.ts` · `app/admin/template-analytics/ExcelUploader.tsx` · `app/admin/template-analytics/page.tsx` | 🔲 **Scope done · รอ implement** |
...
| **EF-2** | **Form Builder UI — DnD canvas** | `app/admin/form-builder/page.tsx` + `FormBuilderClient.tsx` — layout 2-col (FieldPalette ซ้าย + Canvas ขวา) · `FieldPalette.tsx` — 15 field types แบ่งกลุ่ม (Text / Date / Selection / Layout / Special) · Canvas: `@dnd-kit/core` + `@dnd-kit/sortable` — drag field จาก palette ลง canvas, sort ใน canvas, click field เพื่อ config label/placeholder/options · preview render แต่ละ field type inline | ✅ **Done (Session 54)** |
...
| **EF-8** | **Multi-page Preview Carousel** | Carousel 2 หน้าแรกใน modal ทั้ง 3 จุด (`CatalogTemplateList.tsx`, `TemplateListWithPreview.tsx`, `FeaturedTemplateCard.tsx`) · arrow nav + dot indicator + page counter · Fix `preview_pages` storage เพิ่ม `::jsonb` cast (approve-revision, form-builder save/update, wizard action) · Fix preview API regex → allow `*-p{n}-{ts}.jpg` · Fix viewport height 3200→792 (100vh=A4 — blank pages แก้แล้ว) · `Array.isArray` guard ป้องกัน crash จาก stringified data · DB migration fix 1 row · cap 2 pages | ✅ **Done · Live (Session 60 · 2026-05-13)** |

**ลำดับ:** EF-1 → EF-2 → EF-3 → EF-4 → EF-5 → EF-6 → EF-7 → EF-8 ✅ COMPLETE

---

## ADM-RBAC-1 — Custom Admin Auth + Role-Based Access (Session 68) ✅ DONE

> **Done 2026-05-14** — Hybrid 2-tier auth: Supabase (primary) + Custom RBAC (fallback + clerk)
...
| # | Change | Status |
|---|---|---|
| 1 | **PromoCodeBanner** — progress bar + savings hint "ซื้อ 1 ชิ้น ฿30 → ฿XX (ประหยัด X%)" + slots เหลือ (max_uses - used_count) | ✅ Live |
| 2 | **PromoCodeInput (checkout)** — badge `-฿XX` pulse สีเขียวเมื่อ apply code แล้ว | ✅ Live |
| 3 | **PromoCreateForm (admin)** — 💡 hint realtime "ลด ฿XX จากราคา ฿30 → จ่าย ฿XX (-X%)" | ✅ Live |
| 4 | **page.tsx query** — fetch `starts_at, discount_type, discount_value, max_uses, used_count` เพิ่มใน promo query | ✅ Live |

---

## Session 64 Changes (2026-05-13) — RE-1 Report Engine

| # | Change | Status |
|---|---|---|
| 1 | **RE-1 Report Engine — PDF Generator** — engine ที่ 4 ครบ loop · admin กรอกฟอร์ม 8 section → generate PDF → publish → cart/download ระบบเดิม | ✅ Live |
| 2 | **ReportEngineForm** — `app/admin/templates/new/ReportEngineForm.tsx` · wizard mode `engine-report` ใน WizardClient | ✅ Live |
| 3 | **generate-report route** — `app/api/admin/templates/generate-report/route.ts` · puppeteer PDF generation | ✅ Live |
| 4 | **engine-report-types + engine-report** — `lib/engine-report-types.ts` · `lib/engine-report.ts` · PDF HTML builder 8 sections | ✅ Live |
| 5 | **Customer preview card** — `/templates/[slug]` แสดง sky-color preview card สำหรับ report engine | ✅ Live |
| 6 | **Report Revise support** — `ReportReviseForm` + fix crash · admin แก้เนื้อหา report ได้หลัง publish | ✅ Live |
...
| # | Change | Status |
|---|---|---|
| 1 | **Multi-page carousel** arrow nav + dot indicator + page counter ใน modal ทั้ง 3 จุด | ✅ Live |
| 2 | **Fix preview_pages storage** เพิ่ม `::jsonb` cast ใน 4 routes — ป้องกัน jsonb string bug | ✅ Live |
| 3 | **Fix preview API regex** `/api/preview/[filename]` → allow `*-p{n}-{ts}.jpg` (ก่อนหน้านี้ 404 ทุกไฟล์) | ✅ Live |
| 4 | **Fix viewport height** 3200→792px — `100vh=A4` ทำให้ preview pages 2+ ไม่ blank | ✅ Live |
| 5 | **Array.isArray guard** ใน carousel ทั้ง 3 component — ป้องกัน `e.map is not a function` crash | ✅ Live |
| 6 | **DB migration** fix 1 row (`report-budgent-2026`) stringified preview_pages | ✅ Done |
| 7 | **Cap 2 pages** ลด max snapshot 4→2 | ✅ Live |
...
| # | Change | Status |
|---|---|---|
| 1 | **INTEL-C** Section "ครอบคลุมแค่ไหน" — 3-level coverage map (Base/Drill/ก-ฮ) · per-engine coverage % · ✅ matched / 🟠 ยังไม่มี + "+ สร้าง" · เหนือ Google Suggest section | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
...
| 3 | **INTEL-D A** ALPHA_CHARS 6→15 ตัว (เพิ่ม ง จ ต ท บ ป ว ห อ) | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
| 4 | **INTEL-D B** SEED_KEYWORDS 4→8 (เพิ่ม ตาราง/ใบแจ้ง→form · แผนงาน→pipeline · บัญชี→report) merged by engineType | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
| 5 | **INTEL-D C** Section ใหม่ "สร้างอะไรก่อน" — Priority Score = `level × (100−coverage%)` · top-20 uncovered เรียง score · badge 🔴ด่วนสุด/🟠ควรทำ/🟡พิจารณา | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
| 6 | **INTEL-D D** Drill ALL Level 1 ideas (covered + uncovered แทนที่จะ drill เฉพาะ covered) · DRILL_LIMIT 3→6 per engine | ✅ UAT ผ่าน (Session 86 · 2026-05-17) |
