...
| **ADM-FALLBACK-1** | **Emergency Admin Login (Supabase bypass)** | **Scope confirmed (2026-05-13):** admin login ที่ `/admin/login` ยังทำงานได้เมื่อ Supabase ล่ม · **Option A — bcrypt fallback:** `ADMIN_EMAIL` + `ADMIN_PASSWORD_HASH` ใน `.env.local` · try Supabase (3s timeout) → ถ้าล้มเหลว → POST `/api/admin/emergency-login` → verify bcrypt → set httpOnly cookie `_admin_fallback` (JWT 8h) · middleware เช็ค Supabase session OR `_admin_fallback` · rate limit 5 attempts/15min per IP · LINE push แจ้ง owner ทุกครั้งที่ใช้ fallback · **Files NEW:** `app/api/admin/emergency-login/route.ts` · **Files MODIFY:** `app/admin/login/AdminLoginForm.tsx` (เพิ่ม fallback path) · `proxy.ts` หรือ middleware (เช็ค `_admin_fallback` cookie) · `.env.local` (เพิ่ม 2 vars) | 🔴 **Core · Scope Locked** |
