...
| J10 | Wallet แสดง login prompt หลัง download ใน tab เดิม | **Root cause confirmed**: Next.js router cache (client-side) เก็บ /wallet เป็น "not logged in" จากครั้งก่อน · cookies ยังครบ (new tab พิสูจน์แล้ว) · **Fix plan**: เพิ่ม `router.refresh()` ใน DownloadClient เมื่อ state=done + ปุ่ม "กลับกระเป๋า" ที่ใช้ `<a href>` (full reload) แทน Link | 🔴 High |
...
| J12 | LINE OAuth ล้มเหลว — "Error getting user profile from external provider" | **🔴 ยังไม่แก้ได้ (2026-05-08)** — ดู Troubleshooting Log ด้านล่าง | 🔴 Critical |
| J13 | ระบบ Customer Request ขอเทมเพลตด่วน | ลูกค้ากรอก "อยากได้เทมเพลตอะไร" → เก็บใน DB → แอดมินเห็นใน `/admin/requests` → owner สร้าง template ตาม demand จริง · **Plan**: form `/templates/request` + table `template_requests` (topic, detail, contact_line, created_at) + admin list | 🟡 Medium |
...
| DC-8 | **Engine Content Edit + Revision History** | **Design decisions (confirmed 2026-05-09):** (1) เก็บ 1 ไฟล์ล่าสุด — revision ใหม่ replace `pdf_path` เดิม · (2) Revision history = table แยก `template_revisions` · (3) docCode คงเดิมทุก revision (ไม่เปลี่ยน format/เลข) · **DB Migration:** `CREATE TABLE template_revisions (id UUID PK, template_id UUID FK→templates, revision_number INT, engine_data JSONB, pdf_path TEXT, change_note TEXT, created_at TIMESTAMPTZ)` · **Flow:** `/admin/templates/[id]/edit` → ถ้า `engine_type IS NOT NULL` → แสดงปุ่ม "แก้ไขเนื้อหา" + แสดง revision count/history → คลิก → `/admin/templates/[id]/revise` (new page: pre-fill ChecklistEngineForm/PlannerEngineForm จาก `engine_data` ปัจจุบัน → แก้ไข → "Generate PDF Preview (Revision N)" → กรอก change_note → Approve → save revision row + update `templates.engine_data` + `templates.pdf_path`) · **Admin revision list:** `/admin/templates/[id]/revisions` — ตาราง revision_number / created_at / change_note / download PDF link · **docCode:** ดึงจาก `engine_data.s1.docCode` คงเดิมทุก revision ไม่ re-generate · **Files (new):** `migrations/20260509_template_revisions.sql` · `app/admin/templates/[id]/revise/page.tsx` · `app/admin/templates/[id]/revisions/page.tsx` · `app/api/admin/templates/approve-revision/route.ts` · **Files (update):** `app/admin/templates/[id]/edit/page.tsx` (เพิ่ม engine section + revision count) | 🔲 Planned |
