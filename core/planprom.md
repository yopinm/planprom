...
| PROMO-4 | **Auto-Promo Engine Cards** | **Scope (2026-05-11 confirmed):** · **Position:** แถว horizontal 4 cards ใต้ title "Promo Codes" เหนือฟอร์มสร้างโค้ด · **4 Engines:** (1) Slow Week — revenue 7 วันนี้ vs avg 4 สัปดาห์ → signal ถ้า < 60% avg (2) Tier Uplift — % orders paid_item_count=1 → signal ถ้า > 70% (3) Cart Recovery — sessions มี cart ไม่มี order → signal ถ้า abandoned > threshold (4) VPS Break-Even — ฿900/เดือน = ~฿30/วัน → signal เมื่อยอด 7 วันยังขาดเป้า VPS cost · **VPS Engine logic:** ต้องขาย ~129 template/เดือน (฿7/ชิ้น tier floor) หรือ ~43 ออร์เดอร์ (AOV ฿21) → คำนวณ gap → suggest discount size ที่ทำ conversion โดย margin ยังเป็นบวก · **Card UI:** engine name + คำอธิบาย + live signal badge (สีเหลือง "แนะนำสัปดาห์นี้" / สีเทา "ปกติดี") + ปุ่ม [Generate โค้ด] → pre-fill PromoCreateForm · **API:** `GET /api/admin/promo-suggest` — query 4 signals จาก orders/sessions DB → return `[{ engine, signal, reason, suggested }]` · **Files:** `app/api/admin/promo-suggest/route.ts` (NEW) · `app/admin/promo-codes/PromoEngineCards.tsx` (NEW) · `app/admin/promo-codes/page.tsx` (inject cards + pass setter สำหรับ pre-fill) · `app/admin/promo-codes/PromoCreateForm.tsx` (รับ `prefill` prop) | 🔲 Planned — รอ owner confirm |
