...
| **INTEL-E** | **Catalog-Matched Analytics — Per-Category Sales vs Demand** | **Scope (2026-05-14 · pending confirm):** เพิ่ม Section ใหม่ "Catalog Performance" ใน `/admin/template-analytics` — cross-reference ข้อมูลยอดขายกับ `template_categories` จริง (ไม่ใช่แค่ engine_type) · **Section: Catalog Performance table** — category emoji+name · published template count · total sales · total revenue · avg price · gap score (demand สูงแต่ sale ต่ำ) · ปุ่ม "+ เพิ่ม template" ต่อ category · **Cross-reference กับ INTEL-C/D demand signals:** ถ้า category มี keyword match จาก Google Suggest แต่ยอดขาย < 2 → flag badge "📈 Demand สูง ยังไม่มียอด" · **Per-category drill-down:** top-3 templates by revenue per category + zero-sale templates ที่ควร archive/revise · **Query:** JOIN `templates` + `template_category_links` + `template_categories` + `order_items` + `orders` (WHERE status='paid') → GROUP BY category · **Files:** `app/admin/template-analytics/page.tsx` (เพิ่ม Section ใหม่ต่อจาก Market Gap Matrix) | 🔲 **Queued — หลัง INTEL-C/D UAT** |
