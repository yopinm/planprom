...
| **INTEL-E** | **Catalog-Matched Analytics — 4 Smart Features** | **Scope confirmed (2026-05-14):** เพิ่ม intelligence 4 ชั้นใน `/admin/template-analytics` ให้ระบบรู้ว่า "สร้างอะไร → ใส่ catalog ไหน" ต่อยอดจาก Priority List เดิม · **FROZEN:** Google Suggest fetch logic · SEED_KEYWORDS · ALPHA_CHARS · AUDIENCE_RULES · NOISE · S1-S3 KPI/Revenue/Priority sections · DB schema · **Idea 1 — Auto-suggest Catalog ใน S3 Priority Row:** หลัง badge engine_type ทุก row → เพิ่ม "📂 แนะนำ: [category name]" · Logic: `suggestCatalog(idea, categories)` — keyword match ระหว่าง idea string กับ `category.name` + `category.slug` → return closest match · fetch `template_categories` ทั้งหมดใน Phase 1 parallel · **Idea 2 — Catalog Demand Heatmap (Section ใหม่ S6b ระหว่าง Gap Matrix กับ Daily):** ตาราง per-category: emoji+name · template count · Google demand hits (นับ ideas ที่ suggestCatalog match กับ category นี้) · revenue · status badge (📈 Gap สูง / ✅ ดี / 🟠 ยังไม่มี) · ปุ่ม "+ เพิ่ม" per row · **Query:** JOIN templates + template_category_links + template_categories + order_items + orders GROUP BY category · **Idea 3 — Smart "+ สร้าง" Pre-fill:** ปุ่ม "+ สร้าง" ทุกจุดใน S3 Priority + S6b Heatmap → `href="/admin/templates/new?title=...&category=...&engine=..."` · WizardClient รับ searchParams → pre-fill title + category select + engine mode · **Files modify:** `app/admin/templates/new/page.tsx` (รับ searchParams) · `app/admin/templates/new/WizardClient.tsx` (pre-fill state) · **Idea 4 — Per-Catalog Coverage Badge ใน S4:** เพิ่ม tab "แยก Catalog" ใต้ S4 "ครอบคลุมแค่ไหน" · map AUDIENCE_RULES → catalog ด้วย pattern match กับ category name → coverage % per catalog · แสดง: 📂 ชื่อ category · covered X/Y · % badge (🟢≥60 / 🟠≥30 / 🔴<30) · **Files รวม 2 ไฟล์:** `app/admin/template-analytics/page.tsx` (หลัก) · `app/admin/templates/new/WizardClient.tsx` + `page.tsx` (pre-fill) | 🔲 **Queued — หลัง INTEL-C/D UAT** |
