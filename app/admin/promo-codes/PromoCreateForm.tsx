...
export function PromoCreateForm({ suggested }: { suggested: string }) {
  ...
  return (
    <form action={createPromoCodeAction} className="mt-6 rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm space-y-4">
      ...
      {/* Live hint */}
      {hint && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-2.5 text-xs font-semibold text-amber-700">
          ⚠️ {hint}
        </div>
      )}
