        ...
        {excelIdeas.length > 0 && (() => {
          ...
          return (
            <details className="group" open>
              ...
              {/* Gap — client component with tier + filter */}
              {gapItems.length > 0 && (
                <ExcelS0Panel gapItems={gapItems} deleteAction={deleteExcelIdeaAction} />
              )}

              {/* Covered — server-rendered, collapsed */}
                ...
                <details className="mt-5">
                  ...
                  <div className="mt-3 space-y-1.5">
                    {coveredItems.map(item => (
                      <div key={item.id} className="flex items-center gap-2 rounded-lg border border-green-100 bg-green-50 px-3 py-2">
                        ...
                        <span className="flex-1 min-w-0 text-sm text-neutral-700 truncate">{item.idea_text}</span>
                        {item.match && (
                          <Link href={`/admin/templates/${item.match.id}/edit`}
                            className="shrink-0 rounded-full bg-green-200 px-2.5 py-0.5 text-[11px] font-bold text-green-800 hover:bg-green-300">
                            ✅ ดู template
                          </Link>
                        )}
                        <form action={deleteExcelIdeaAction}>
                          <input type="hidden" name="id" value={item.id} />
                          <button type="submit" title="ลบ" className="shrink-0 rounded px-1.5 py-0.5 text-[11px] text-neutral-400 hover:bg-red-50 hover:text-red-500 transition">✕</button>
                        </form>
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="divide-y divide-neutral-50">
                      {sellers.map((r, i) => (
                        <div key={r.id} className="flex items-center gap-3 px-5 py-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-neutral-800 truncate">{r.title}</p>
                            <div className="mt-1 flex items-center gap-2">
                              <div className="h-1 w-16 rounded-full bg-neutral-100">
                                <div className="h-1 rounded-full bg-indigo-400" style={{ width: `${Math.round((Number(r.paid_count) / groupMax) * 100)}%` }} />
                              </div>
                              <span className="text-[10px] text-neutral-400">{r.paid_count} orders · ฿{Number(r.revenue).toLocaleString()} · {r.downloads} DL</span>
                            </div>
                          </div>
                          <Link href={`/admin/templates/${r.id}/edit`} className="shrink-0 rounded-xl border border-neutral-200 px-3 py-1.5 text-[10px] font-black text-neutral-500 hover:border-indigo-400 hover:text-indigo-600 transition">แก้ไข</Link>
                        </div>
                      ))}
                    </div>
                  )}
                  {zeros.length > 0 && (
                    <div className="bg-red-50/50 px-5 py-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {zeros.map(r => (
                          <Link key={r.id} href={`/admin/templates/${r.id}/edit`} className="rounded-full border border-red-200 bg-white px-2.5 py-1 text-[10px] font-bold text-red-600 hover:bg-red-50 transition">{r.title}</Link>
                        ))}
                      </div>
                    </div>
                  )}
